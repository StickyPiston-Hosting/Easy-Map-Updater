...
def handle_motion_modification(command: list[str], is_macro: bool) -> str:
...
def handle_teleport(command: list[str], is_macro: bool) -> str:
    ...
    command_string = " ".join(command[:index]) + teleport_dismount.handle_teleport(command[index:], is_macro)
